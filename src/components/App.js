...
class App extends Component {
  ...
  async loadBlockchainData() {
    ...
    // Load Contracts
    await this.loadTetherData(web3);
    await this.loadRwd(web3);
    await this.loadDecentralBankData(web3);
    this.setState({ loading: false });
  }

  async loadTetherData(web3) {
    const tetherData = Tether.networks[this.state.networkId];
    if (tetherData) {
  ...
  async loadRwd(web3) {
    const rwdData = RWD.networks[this.state.networkId];
    if (rwdData) {
  ...
  async loadDecentralBankData(web3) {
    const decentralBankData = DecentralBank.networks[this.state.networkId];
